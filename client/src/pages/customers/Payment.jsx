...
function Payment() {
  ...
  const totalCartAmount = 
    ...
    ?
    cartProducts.reduce((sum , item) => sum + (item?.SalePrice > 0 ? item.SalePrice : item.Price) * item.Quantity , 0).toFixed(2)
    :
    0;
        ...
        <div className='flex flex-col gap-4'>
          {
            cartProducts && cartProducts.length > 0 &&
              cartProducts.map((item,index) => 
                <Cart_Tiles key={index} cartItem={item}/>
            )
          }
          </div>
          <div className="mt-2 space-y-4">
            <div className="flex flex-col justify-between">
              ...
              <div className='flex justify-between mt-2'>
                ...
                <span className="text-gray-600 mb-1"> 10.00 MAD</span>
              </div>
              <Separator />
              <div className='flex justify-between mt-2'>
          ...
          <div className="w-full" >
            <Button onClick={() => paymentRedirector()} className="w-full">Checkout with &nbsp;<img width="45px" src="/stripe.png" /></Button>
          </div>
        </div>
        </div>
      </div>
